...
class OpenSearchDataSourceTest {
    ...
    private static Map<String, Object> getDatasourceConfig() {
        return Map.of("https", false,
                "port", OPENSEARCH.getMappedPort(9200),
                "host", "localhost",
                "username", "admin",
                "password", "admin");
    }
    ...
    void testAsset() throws Exception {
        final Map<String, Object> assetConfig = Map.of(
                "index-name",
                "test-index",
                "settings",
                """
                ...
                """
                        ...
                        """,
                "datasource", Map.of("configuration", getDatasourceConfig())
        );
    ...
    private AssetManager createAssetManager(Map<String, Object> config) throws Exception {
        final AssetManager instance = new OpenSearchAssetsManagerProvider()
                .createInstance("opensearch-index");
        ...
        try (final OpenSearchWriter.OpenSearchVectorDatabaseWriter writer =
            ...
            writer.initialise(
                    Map.of(
                            ...
                            "id", "key.name",
                            "bulk-parameters", Map.of("refresh", "true")
                    )
            );
            SimpleRecord record =
                    SimpleRecord.of(
            ...
            final List<Map<String, Object>> result = writer.getDataSource()
                    .fetchData("""
                            {
    ...
    void testWriteVectors() throws Exception {
        ...
        try (final OpenSearchWriter.OpenSearchVectorDatabaseWriter writer =
            ...
            assertEquals("This is a question", ((Map) result.get(0).get("document")).get("original"));
            assertEquals(List.of(1.0d, 2.0d, 3.0d), ((Map) result.get(0).get("document")).get("my_vector1"));
            assertEquals("test-index-000", result.get(0).get("index"));
        }
