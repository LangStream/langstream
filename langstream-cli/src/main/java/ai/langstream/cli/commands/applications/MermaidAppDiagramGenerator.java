...
public class MermaidAppDiagramGenerator {
    ...
    public static String generate(String applicationDescription) {
        ...
        final Map<String, Object> application =
                (Map<String, Object>) map.get("application");
        ...
        model.getResources()
                .forEach(r -> {
                    final String label = r.getType().equals("datasource") ? "[(\"" + r.getLabel() + "\")]" : "[\"Service: " + r.getLabel() + "\"]";
        ...
        model.getTopics()
                .forEach(t -> mermaid.append(t).append("\n"));
        model.getPipelines()
                .forEach(p -> {
                    p.getAgents()
                            .forEach(a -> {
                                if (a.getInputTopic() != null) {
                                    mermaid.append(a.getInputTopic()).append(" --> ")
                                            .append(a.getId())
                                            .append("\n");
                                }
                            });
        ...
        model.getPipelines()
                .forEach(p -> {
                    mermaid.append("subgraph ").append(p.getId()).append("[\"<b>").append(p.getId())
                            .append("</b>\"]\n");
                    p.getAgents()
                            .forEach(a -> {
                                if (a.getOutput() != null) {
                                    for (String output : a.getOutput()) {
                                        mermaid.append(a.getId()).append("(\"").append(a.getLabel()).append("\")")
                                                .append(" --> ")
                                                .append(output)
                                                .append("\n");
                                    }
    ...
    private static void applyResources(ApplicationModel model, Map<String, Object> application) {
        ((Map<String, Object>) asMap(application.get("resources")))
                ...
                .forEach(entry -> {
                    ...
                    model.getResources().add(new Resource(entry.getKey(), (String) resource.get("name"), type, (String) resource.get("type")));
                });
    }
                ...
                .forEach(module -> {
                    ...
                    asList(asMap(module).get("pipelines"))
                            .forEach(p -> {
                                ...
                                for (Object ag : asList(pipeline.get("agents"))) {
                                    ...
                                    if (input != null) {
                                        if (input.get("connectionType").equals("TOPIC")) {
                                            agent.setInputTopic((String) input.get("definition"));
                                        }
                                    } else {
                                        agent.setExternalInput(model.getSourceExternal());
                                    }
                                    ...
                                    if (output != null) {
                                        outputs.add((String) output.get("definition"));
                                    }
                                    ...
                                    switch (agentType) {
                                        ...
                                        case "compute-ai-embeddings":
                                            if (agentConfig.containsKey("ai-service")) {
                                                outputs.add((String) agentConfig.get("ai-service"));
                                            } else {
                                                final Resource resource = model.getResources()
                                                        .stream()
                                                        .filter(r -> r.getOriginalType().equals("vertex-configuration")
                                                                     ||
                                                                     r.getOriginalType().equals("open-ai-configuration")
                                                                     ||
                                                                     r.getOriginalType()
                                                                             .equals("hugging-face-configuration"))
                                                        .findFirst()
                                                        .orElse(null);
                                                if (resource != null) {
                                                    outputs.add(resource.getId().replace(" ", ""));
                                                }
                                            }
                                            break;
                                        default:
                                ...
                                model.getPipelines().add(new Pipeline((String) pipeline.get("id"), agents));
                            });
                });
    }

    private static void applyTopics(ApplicationModel model, Map<String, Object> module) {
        asList(module.get("topics")).forEach(t -> model.getTopics().add((String) asMap(t).get("name")));
    }
...
// TODO: handle chat
                model.getGateways().add(new Gateway(
                        (String) gw.get("id"),
