    ...
    public List<SourceRecordAndResult> process(List<Record> records) {
        ...
        while (current < processors.size()) {
            ...
            for (SourceRecordAndResult entry : currentResults) {
                ...
                Record sourceRecord = entry.sourceRecord();
                List<Record> sinkRecords = entry.resultRecords();
                if (entry.error() != null) {
                    ...
                    try {
                        ...
                        nextStageResults.add(new SourceRecordAndResult(sourceRecord,
                                processed
                                        .stream()
                                        .map(SourceRecordAndResult::resultRecords)
        ...
        long countOut = currentResults
                .stream()
                .filter(s -> s.resultRecords() != null)
                .mapToInt(s -> s.resultRecords().size())
                .sum();
